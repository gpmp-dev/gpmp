...
def sample_from_selection_criterion(
    ...
    info : object, optional
        Must have attributes:
          - covparam: initial GP parameters (array-like).
          - selection_criterion_nograd: returns the negative log-posterior.
        Not used if custom_log_target is provided.
    custom_log_target : callable, optional
        Custom log-target function. If provided, info must not be provided.
    ...
    random_init : bool, optional
        If True, initialize chains randomly within init_box.
    init_box : list, optional
        Domain for random initialization: [[lower bounds], [upper bounds]].
    silent : bool, optional
        If True, suppress messages.
    show_progress : bool, optional
        If True, display progress information.
    ...
    mh : MetropolisHastings
        The sampler instance.
    """
    # Branch 1: custom_log_target is provided (info must not be provided)
    if custom_log_target is not None:
        if info is not None:
            raise ValueError("When custom_log_target is provided, do not provide info.")
        log_target = custom_log_target
        if not random_init and param_initial_states is None:
            raise ValueError(
                "Without info, param_initial_states must be provided when random_init is False."
            )
        if random_init:
            if init_box is None:
                raise ValueError("init_box must be provided for random initialization.")
            if isinstance(init_box, (list, tuple)) and len(init_box) == 2:
                ...
                if isinstance(lower, (list, np.ndarray)):
                    ...
                    raise ValueError(
                        "For random_init with custom_log_target, init_box bounds must be lists."
                    )
                else:
                    raise ValueError("Invalid init_box format.")
            else:
                raise ValueError("init_box must be of the form [lower, upper].")
            param_initial_states = randunif(dim, n_chains, init_box)
        else:
            dim = param_initial_states.shape[1]
    # Branch 2: info is provided (and custom_log_target is None)
    else:
        if info is None:
            raise ValueError("Either info or custom_log_target must be provided.")
        if not hasattr(info, "covparam"):
        ...
        if not callable(info.selection_criterion_nograd):
            raise TypeError("info.selection_criterion_nograd must be callable.")
        log_target = lambda p: -info.selection_criterion_nograd(p)
        # Priority to param_initial_states if provided
        if random_init:
            if init_box is None:
                raise ValueError("init_box must be provided for random initialization.")
            if isinstance(init_box, (list, tuple)) and len(init_box) == 2:
    ...
    options = MHOptions(
        ...
        n_chains=n_chains,
        target_acceptance=0.3,
        proposal_distribution_param_init=0.1 * np.ones_like(param_initial_states[0]),
        adaptation_method="Haario",
        adaptation_interval=50,
        haario_adapt_factor_burnin_phase=1.0,
        haario_adapt_factor_sampling_phase=0.5,
