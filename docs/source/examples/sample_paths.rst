...
noise-free observations. Conditional paths are useful for simulation studies,
uncertainty visualization, and algorithms that require full random functions
rather than only posterior means and variances.
...
The sample paths should pass through the noise-free observations. Away from the
...
* Use sample paths when an algorithm needs random functions, not only pointwise
